...
#df_dirty[df_dirty["team"] == "Spirit"].to_csv("abc0.csv", index=False)
...
with open("bets.csv", "w", newline="", encoding="utf-8") as f: 
    for i, row in odds.iterrows():
        ...
        def write(t1, t2, p1_book, p1_model, bet):
        ...
        if (p1_model > p1_book + 0.05):
            if (p1_book < 0.5):
                ...
                if (score1 > score2):
                    money += 35 * 1 / p1_book
                    f.write("won ")
                write(t1, t2, p1_book, p1_model, 35)
                n+=1
            if (p1_book > 0.5):
                money -= 65
                if (score1 > score2):
                    money += 65 * 1 / p1_book
                    f.write("won ")
                write(t1, t2, p1_book, p1_model, 65)
                n+=1
            ...
            if (p1_book < 0.5):
                ...
                if (score1 > score2): 
                    money += 5 * 1 / p1_book
                    f.write("won ")
                write(t1, t2, p1_book, p1_model, 5)
                n+=0.2
            if (p1_book > 0.5):
                money -= 20
                if (score1 > score2): 
                    money += 20 * 1 / p1_book
                    f.write("won ")
                write(t1, t2, p1_book, p1_model, 20)
                n+=0.2
            ...
            if (p2_book < 0.5):
                ...
                if (score1 < score2): 
                    money += 35 * 1 / p2_book
                    f.write("won ")
                write(t2, t1, p2_book, p2_model, 35)
                n+=1
            if (p2_book > 0.5):
                money -= 65
                if (score1 < score2): 
                    money += 65 * 1 / p2_book
                    f.write("won ")
                write(t2, t1, p2_book, p2_model, 65)
                n+=1
            ...
            if (p2_book < 0.5):
                ...
                if (score1 < score2):
                    money += 5 * 1 / p2_book
                    f.write("won ")
                write(t2, t1, p2_book, p2_model, 5)
                n+=0.2
            if (p2_book > 0.5):
                money -= 20
                if (score1 < score2):
                    money += 20 * 1 / p2_book
                    f.write("won ")
                write(t2, t1, p2_book, p2_model, 20)
                n+=0.2
        else: 
